...
% Thezata ltcaption package supports \CaptionLabelFont & \CaptionTextFont introduced by the NTG document classes
...
\promotor{dr inż. Marek Piasecki, W4/K-9}
\date{WROCŁAW, 2018}
...
dla Pana dr inż. Marka Piaseckiego za możliwość realizacji niniejszej pracy, poświęcony czas oraz wiele cennych porad 
  
\\[0.5in]
oraz dla wszystkich osób, które wsparły rozwój platformy poprzez udział w testach użyteczności.
...
% moj fix ktory robi wciecia w nastepujacym po nich spisach 
\makeatletter
\renewcommand*\l@figure{\@dottedtocline{1}{1.5em}{2.3em}}% Default: 1.5em/2.3em
...
\renewcommand*{\lstlistingname}{Fragmentkodu}
\renewcommand{\lstlistingname}{Fragmentkodu}
...
\include{01-wstep/01-wstep}
\include{02-istniejace-rozwiazania/02-istniejace-rozwiazania}
\include{03-koncept/03-koncept}
\include{04-projekt/04-projekt}
\include{05-architektura-i-technologie/05-architektura-i-technologie}
\include{06-implementacja/06-implementacja}
\include{07-walidacja/07-walidacja}
\include{065-dzialanie/065-dzialanie}	
\include{08-perspektywy-rozwoju/08-perspektywy-rozwoju}
\include{09-podsumowanie/09-podsumowanie}
